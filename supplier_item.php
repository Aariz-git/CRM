...
if (isset($_POST["upload"])) {
    if ($_FILES["supply_item_csv"]["name"]) {
        $filename = explode(".", $_FILES["supply_item_csv"]["name"]);
        if (end($filename) == "csv") {
            $handle = fopen($_FILES['supply_item_csv']['tmp_name'], "r");
            while ($data = fgetcsv($handle)) {
                ...
                $query = "INSERT INTO `supplier_item`(`segment`, `supplier`, `product`, `customer`, `quantity`) VALUES ('$segment','$supplier','$product','$customer','$quantity')";
...
if (isset($_POST['delete_btn'])) {
    $id = $_POST['del_id'];
    $query = "Delete from supplier_item where id = '$id'";
    $query_run = mysqli_query($con, $query);
    if ($query_run) {
		echo "de";
            ...
            <div class="main-content">
                ...
                <div class="container-fluid">
                    <div class="row" id="Admin_Table">
                        <?php
                        $query = "Select * from supplier_item";
                        $query_run = mysqli_query($con, $query);
                        ?>
                        <div class="col-lg-12">
                            <h2 class="title-1 m-b-25">Supplier Item <input type='button' id='printData' class="btn btn-success ml-5" value='Print' onclick='printData();'></h2>
                            <form method="POST" enctype="multipart/form-data" class="pull-left m-2">
                                <input type="file" name="supply_item_csv" accept=".csv">
                            ...
                            <a href="Add_supplier_item.php" class="btn btn-success pull-right m-2">Add Supplier Item</a>
                            <?php echo $msg ?>
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo $row['quantity'] ?></td>
														...
														<td id="delHide">																						                                                            
														<form id="editHide" action="edit_supplier_item.php" method="post">
                                                                <input type="hidden" name="edit_id" value="<?php echo $row['id']; ?>">
                                                                <button type="submit"   name="edit_btn" class="btn btn-success">Edit</button>
                                                            </form>
															</td>
															<td id="delHide">
                                                            <form action="supplier_item.php" method="POST">
  ...
  $('td:nth-child(7),td:nth-child(8),th:nth-child(7),th:nth-child(8)').show();
            ...
            $("#search").on("keyup",function(){
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {'search' : search_term, 'table_name': 'supplier_item'},
