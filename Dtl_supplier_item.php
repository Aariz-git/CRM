...
if (isset($_POST["upload"])) {
    if ($_FILES["dtl_supply_item_csv"]["name"]) {
        $filename = explode(".", $_FILES["dtl_supply_item_csv"]["name"]);
        if (end($filename) == "csv") {
            $handle = fopen($_FILES['dtl_supply_item_csv']['tmp_name'], "r");
            while ($data = fgetcsv($handle)) {
                ...
                $query = "INSERT INTO `dtl_supplier_item`(`segment`, `supplier`, `product`, `customer`, `quantity`, `price`) VALUES ('$segment','$supplier','$product','$customer','$quantity','$price')";
...
if (isset($_POST['delete_btn'])) {
    $id = $_POST['del_id'];
    $query = "Delete from dtl_supplier_item where id = '$id'";
    $query_run = mysqli_query($con, $query);
    if ($query_run) {
		echo "de";
        header('Location: Dtl_supplier_item.php');
	}
}
?>
...
<head>
    ...
    <!-- Title Page-->
    <title>Detail of Supplier Item</title>
            ...
            <div class="main-content">
                ...
                <div class="container-fluid">
                    <div class="row" id="Admin_Table">
                        <?php
                        $query = "Select * from dtl_supplier_item";
                        $query_run = mysqli_query($con, $query);
                        ?>
                        <div class="col-lg-12">
                            <h2 class="title-1 m-b-25">Details of Supplier Item <input type='button' id='printData' class="btn btn-success ml-5" value='Print' onclick='printData();'></h2>
                            <form method="POST" enctype="multipart/form-data" class="pull-left m-2">
                                <input type="file" name="dtl_supply_item_csv" accept=".csv">
                            ...
                            <a href="Add_dtl_supplier_item.php" class="btn btn-success pull-right m-2">Add Detail of Supplier Item</a>
                            <?php echo $msg ?>
                                    ...
                                    <tbody id="table_data">
                                        ...
                                        ?>
                                                <tr>
													...
													<td id="delHide">																						                                                            
													<form id="editHide" action="edit_dtl_supplier_item.php" method="post">
                                                                <input type="hidden" name="edit_id" value="<?php echo $row['id']; ?>">
                                                                <button type="submit"   name="edit_btn" class="btn btn-success">Edit</button>
                                                            </form>
															</td>
															<td id="delHide">
                                                            <form action="Dtl_supplier_item.php" method="POST">
...
$('td:nth-child(8),td:nth-child(9),th:nth-child(8),th:nth-child(9)').hide();
   ...
   $("#actionHide").show();
		...
		$('th').css("border","0px");
		$('td').css("border","0px");		
  $('td:nth-child(8),td:nth-child(9),th:nth-child(8),th:nth-child(9)').show();
}
            ...
            $("#search").on("keyup",function(){
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {'search' : search_term, 'table_name': 'dtl_supplier_item'},
