...
if (isset($_POST["upload"])) {
    if ($_FILES["buy_item_csv"]["name"]) {
        $filename = explode(".", $_FILES["buy_item_csv"]["name"]);
        if (end($filename) == "csv") {
            $handle = fopen($_FILES['buy_item_csv']['tmp_name'], "r");
            while ($data = fgetcsv($handle)) {
                ...
                $query = "INSERT INTO `buyer_item`(`segment`, `buyer`, `product`, `customer`, `quantity`) VALUES ('$segment','$buyer','$product','$customer','$quantity')";
            ...
            <div class="main-content">
                ...
                <div class="container-fluid">
                    <div class="row" id="Admin_Table">
                        <?php
                        $query = "Select * from buyer_item";
                        $query_run = mysqli_query($con, $query);
                        ?>
                        <div class="col-lg-12">
                            <h2 class="title-1 m-b-25">Buyer Item <input type='button' id='printData' class="btn btn-success ml-5" value='Print' onclick='printData();'></h2>
                            <form method="POST" enctype="multipart/form-data" class="pull-left m-2">
                                <input type="file" name="buy_item_csv" accept=".csv">
                            ...
                            <a href="Add_buyer_item.php" class="btn btn-success pull-right m-2">Add Buyer Item</a>
                            <?php echo $msg ?>
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo $row['quantity'] ?></td>
															...
															<td id="">																									
                                                            <form id="editHide" action="Edit_buyer_item.php" method="post">
                                                                <input type="hidden" name="Buyer_edit_id" value="<?php echo $row['id']; ?>">
                                                                <button type="submit"   name="Buyer_edit_btn" class="btn btn-success">Edit</button>
                                                            </form>
                                                        </td>
															<td id="delHide">
                                                            <form action="Buyer_Item.php" method="POST">
                                                                <input type="hidden" name="Buyer_del_id" value="<?php echo $row['id']; ?>">
                                                                <button type="submit" name="Buyer_Item_delete_btn" class="btn btn-danger">Delete</button>
                                                            </form>
                                                        </td>
                                                </tr>
                                        <?php
   ...
   $('td:nth-child(7),td:nth-child(8),th:nth-child(7),th:nth-child(8)').show();
            ...
            $("#search").on("keyup",function(){
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {'search' : search_term, 'table_name': 'buyer_item'},
